...
func (r *router) HandleFunc(method, pattern string, h HandlerFunc) {
	...
	// method = "GET" or "POST", pattern = /users/ ... 등
	...
	m[pattern] = h // r.handlers[method][pattern]http.HandlerFunc = 매개변수로 받은 함수 할당
}

func (r *router) handler() HandlerFunc {
	return func(c *Context) {
		// http 메서드에 맞는 모든 handler를 반복하며 요청 URL에 해당하는 handler를 찾음
